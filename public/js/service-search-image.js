...
(function () {
    ...
    function imageExistIn(imageUrl, list) {
        if (!list || !list.length) {
            return false;
        }
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (typeof list[i] === 'object') {
                tocompare = list[i].imageUrl;
            }
            else {
                tocompare = list[i];
            }

            if (tocompare == imageUrl) {
                return true;
            }
        }
    ...
    function getResultLi(item, imageClick, heroOnClick, compares) {

        var li  = document.createElement('li');
        li.className = 'exp-image masonryImage masonry-item';
        li.setAttribute('data-link', item.link);
        
        var img = document.createElement('img');
        //img.src = rs.items[i].image.thumbnailLink;
        ...
        img.className = 'exp-image-img';

        img.setAttribute('data-link', item.link);
        img.setAttribute('data-caption', item.title);
        img.setAttribute('data-page', item.image.contextLink);
        img.setAttribute('data-source', item.displayLink);
        img.setAttribute('data-height', item.image.height);
        img.setAttribute('data-width', item.image.width);
        img.setAttribute('data-size', item.image.byteSize);

        img.width  = 400;
        img.height = (item.image.height * (400 / item.image.width));
        // img.style.width  = '400px';
        //img.style.height = (item.image.height * (400 / item.image.width)) + 'px'; 

        $(img).error(function () {
            $(this).parent().remove();
        });

        /*var popoverImg = '<img src="' + item.link + '">';
        $(img).popover({trigger: 'hover', container: 'body', content: popoverImg, html: true, delay: 1500});*/

        var mark = document.createElement('span');
        mark.className = 'exp-image-mark';    
        ...
        if (heroOnClick) {
            ...
            heroBtn.className = 'exp-image-hero exp-image-fave';
            heroBtn.innerHTML = '<i class="fa fa-star-o"></i>';
            heroBtn.onclick   = heroOnClick;
            
            li.appendChild(heroBtn);
        }

        if (typeof imageClick === 'function') {
            // assign on click
            li.onclick = imageClick;
        }

        /*
        if (imageExistIn(item.link, exp.experienceImages)) {
            $(li).addClass('active');
        }*/

        if (compares && compares.length) {
            if (imageExistIn(item.link, compares)) {
                $(li).addClass('active');
            }
        }
    ...
    //
    window.imageSearchCreateResultLi = getResultLi;
...
(function () {
    ...
    var _imageLegacyCallback;

    function imageSearchLegacy(keyword, offset, callback) {
        _imageLegacyCallback = callback;
        ...
        if (offset == 0) {
            ...
            _imageLegacyPage ++;
            imageSearch.gotoPage(_imageLegacyPage);
        }
    };

    function imageSearchLegacyComplete() {
        ...
        if (imageSearch.results && imageSearch.results.length > 0) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                tmp.link  = results[i].url;
                tmp.title = results[i].titleNoFormatting;
                tmp.image = {};
                tmp.image.contextLink = results[i].originalContextUrl;
                tmp.displayLink = results[i].visibleUrl;
                tmp.image.height = results[i].height;
                tmp.image.width  = results[i].width;
                tmp.image.byteSize = 0; // not available

                rs.push(tmp);
            }

            _imageLegacyCallback({items:rs}, imageSearch.cursor);
        } 
        else {

            _imageLegacyCallback({items: []}, imageSearch.cursor);
        }
    }
    ...
    google.setOnLoadCallback(function () {
        ...
        imageSearch.setSearchCompleteCallback(this, imageSearchLegacyComplete, null);
    });


    window.imageSearchLegacy = imageSearchLegacy;
...
(function () {
    ...
    var flickr = new Flickr({
        api_key: "d5a9a975e20b6f37234c15e8a2529a2e"
    });

    var _flickrPage = 1;

    function searchFlickr(searchKey, reset, callback) {
        ...
        if (reset) {
            ...
            // remove old pic
            $('#exp-edit-image-list-flickr li').each(function () {
            ...
            function(err, result) {
                ...
                if (result && result.photos && result.photos.photo) {
                    ...
                    for (var i = 0; i < result.photos.photo.length; i++) {
                        ...
                        var tmp = {};
                        tmp.link  = 'https://farm' + p.farm + '.staticflickr.com/' + p.server + '/' + p.id + '_' + p.secret + '_b.jpg';
                        tmp.thumbnailLink = 'https://farm' + p.farm + '.staticflickr.com/' + p.server + '/' + p.id + '_' + p.secret + '_m.jpg';
                        tmp.title = p.title;
                        tmp.image = {};
                        tmp.image.contextLink = ('https://flickr.com/photos/' + p.owner + '/' + p.id);
                        tmp.displayLink = ('https://flickr.com/' + p.owner);
                        tmp.image.height = 0;
                        tmp.image.width  = 0;
                        tmp.image.byteSize = 0; // not available 
                    ...
                    // renderImageResults({items: rs}, 0, '#exp-edit-image-list-flickr > ul');
                }
            }
        );
    }

    window.searchFlickr = searchFlickr;

})();

(function () {
    //////////// instagram
    // var instagramKey = '1319.f59def8.5b99fabfdbbe458c8411d9d4d18ce045';
    var instagramKey = '9c539527f4dd454ca28f2916b7c95ee4';

    function searchInstagram(lat, lon, callback) {
        
        var url = 'https://api.instagram.com/v1/media/search?lat=' + lat + '&lng=' + lon + '&client_id=' + instagramKey + '&count=40&callback=?';
    ...
    // search
    function searchInstagramByLocation(locId, nextUrl, callback) {
        
        if (!nextUrl) {
            url = 'https://api.instagram.com/v1/locations/' + locId + '/media/recent?client_id=' + instagramKey + '&count=40&callback=?';
        }
        else {
            url = locId + '&callback=?';
        }
    ...
    function searchInstagramNearby(lat, lon, callback) {
        var url = 'https://api.instagram.com/v1/locations/search?lat=' + lat + '&lng=' + lon + '&client_id=' + instagramKey + '&count=40&distance=5000&callback=?';
    ...
    window.searchInstagramNearby = searchInstagramNearby;
    window.searchInstagramByLocation = searchInstagramByLocation;
    window.searchInstagram = searchInstagram;

    // window.setSearchInstagramCallback = setSearchInstagramCallback;
